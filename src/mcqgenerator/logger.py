...
# os.path.join(os.getcwd(), "logs"): Combines the current working directory with the "logs" directory name to form a full path for where the log files will be stored.
...
#level=logging.INFO: Sets the logging level to INFO, meaning that all messages with a severity level of INFO or higher (e.g., WARNING, ERROR, CRITICAL) will be logged.
#format="[%(asctime)s] %(lineno)d %(name)s - %(levelname)s - %(message)s": Defines the format of each log message.
